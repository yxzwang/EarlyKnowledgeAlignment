...
class ActorRolloutRefWorker(MegatronWorker):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        log_gpu_memory_usage('After recompute log prob', logger=logger)
        return output
# # 在 megatron_workers.py 的 ActorRolloutRefWorker 类中

#     @register(dispatch_mode=Dispatch.MEGATRON_COMPUTE_PROTO)
#     def compute_log_prob(self, data: DataProto):
#         raise ValueError("--- 错误：Megatron Worker 被调用了！---")
#         assert self._is_actor
#         data = data.to('cuda')
#         output = data
#         # we should always recompute old_log_probs when it is HybridEngine
...
#         # ★★★ 1. self.actor.compute_log_prob 现在返回一个字典 ★★★
#         result_dict = self.actor.compute_log_prob(data=output)
        
#         # ★★★ 2. 从字典中分别取出 log_probs 和 logits ★★★
#         old_log_probs = result_dict['log_probs']
#         logits = result_dict['logits']
        
#         # ★★★ 3. 将它们都添加到返回的 DataProto 中 ★★★
#         output.batch['old_log_probs'] = old_log_probs
#         output.batch['logits'] = logits
