...
class MegatronPPOActor(BasePPOActor):
    ...
    # 在 megatron_actor.py 的 MegatronPPOActor 类中

    # def compute_log_prob(self, data: DataProto) -> Dict[str, torch.Tensor]: # ★★★ 1. 修改返回类型提示 ★★★
    #     """Compute the log probability of the responses given input_ids, attention_mask and position_ids
    ...
    #     Returns:
    #         Dict[str, torch.Tensor]: A dictionary containing 'log_probs' and 'logits' tensors.
    #     """
    #     data.batch = data.batch.contiguous()
    ...
    #         response_length = response.size(1)
    #         logits = output['logits']
    #         # ★★★ 2. 在这个内部函数中，logits 已经存在 ★★★
    #         logits_for_logprobs = logits[:, -response_length - 1:-1]
    #         log_probs = vocab_parallel_log_probs_from_logits(logits_for_logprobs, response)
    #         # ★★★ 3. 让内部函数同时返回 log_probs 和完整的 logits ★★★
    ...
    #                 log_probs = torch.cat([o['log_probs'] for o in output], dim=0)  # (bs, seq_size)
    #                 log_probs = log_probs.to(torch.float32)
    #                 # ★★★ 4. 同样地，拼接得到完整的 logits 张量 ★★★
    #                 logits = torch.cat([o['logits'] for o in output], dim=0)
    #                 logits = logits.to(torch.float32) # 或者您需要的其他类型
    #             else:
    #                 log_probs = torch.empty(size=(batch_size, response_length),
    #                                         dtype=torch.float32,
    #                                         device=input_ids.device)
    #                 # ★★★ 5. 为 logits 创建一个空的占位张量 ★★★
    #                 # 注意：需要知道 vocab_size
    #                 vocab_size = self.model_config.vocab_size
    #                 seq_len = input_ids.size(1)
    #                 logits = torch.empty(size=(batch_size, seq_len, vocab_size),
    #                                      dtype=torch.float32, # 保持和上面一致
    #                                      device=input_ids.device)
    ...
    #     # ★★★ 7. 返回一个包含 log_probs 和 logits 的字典 ★★★
    #     return {'log_probs': log_probs, 'logits': logits}
