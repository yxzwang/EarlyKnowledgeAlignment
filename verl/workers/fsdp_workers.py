...
# # 在 fsdp_workers.py 的 ActorRolloutRefWorker 类中

#     @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
#     def compute_log_prob(self, data: DataProto):
#         # raise ValueError("--- 正确：DP/FSDP Worker 被调用了！---") # ★★★ 1. 移除或注释掉这行 ★★★
#         assert self._is_actor
#         if self._is_offload_param:
#             load_fsdp_model_to_gpu(self.actor_module_fsdp)
#         data = data.to('cuda')
...
#             # ★★★ 2. self.actor.compute_log_prob 现在返回一个字典 ★★★
#             result_dict = self.actor.compute_log_prob(data=data)
            
#             # ★★★ 3. 用这个字典来构建 DataProto，包含两个键 ★★★
#             output = DataProto.from_dict(tensors={'old_log_probs': result_dict['log_probs'], 'logits': result_dict['logits']},
#                                          meta_info={'temperature': self.config.rollout.temperature})
